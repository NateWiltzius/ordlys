...
import {
  getEditableDeckVocabularyForSearchData,
  getEditableLessonVocabularyData,
...
export async function GET(request: NextRequest, { params }: Context) {
  ...
  try {
    const vocabs =
      lessonIdValue === null
        ? await getEditableDeckVocabularyForSearchData(deckId)
        : await getEditableLessonVocabularyData(deckId, Number(lessonIdValue));

    return NextResponse.json(vocabs.map(serializeVocab), {
