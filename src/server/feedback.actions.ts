...
export async function createFeedbackAction(input: CreateFeedbackInput): Promise<void> {
  if (!input || typeof input !== 'object') throw new Error('Invalid feedback.');
  if (!FEEDBACK_CATEGORIES.has(input.category)) throw new Error('Choose a feedback type.');
